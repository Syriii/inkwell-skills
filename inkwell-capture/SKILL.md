...
| 方式 | 适用场景 |
|------|---------|
| **脚本**（`web_fetch.py` / `web_fetch_full.py` / `forum_scraper.py`） | **所有场景首选**。脚本内部自动处理渲染降级（requests → Playwright），不需要我来决定 |
| **MCP Playwright 浏览器** | **仅两个例外**：①知乎（脚本全 403，必须真实浏览器 + 已登录 session）②煎蛋评论区（JS 动态加载，脚本拿不到） |

> 每当你伸手拿 MCP 浏览器之前，先看域名规则：这个站脚本能不能搞定？脚本搞不定的才走浏览器。
>
