...
| URL 模式 | 含义 | 采集范围 |
|---------|------|---------|
| `/question/{id}/answer/{aid}` 或 `/answer/{id}` | 单个回答 | 仅该回答 + 该回答的评论区 |
...
├── {问题-slug}.md              # 问题概览（标题、描述、标签、关注数、浏览数）
├── {回答1-作者-slug}/
│   ├── {回答1-作者-slug}.md    # 回答正文
│   ├── comments.md             # 该回答的评论区
│   └── images/                 # 该回答的图片
...
**说明**：
- `问题-slug` 从问题标题生成（如 `拐卖人口罪为什么会改为拐卖妇女儿童罪`）
- `回答-作者-slug` 从作者名生成（如 `WHO文艺编辑部`）
- 每个回答独立一个子目录，正文、评论、图片各自隔离
- 图片永远放在对应回答的 `images/` 下
- 评论区默认上限由 `comment_limit` 控制（默认 500 条），超过时先询问用户
