...
| L2 | MCP Playwright 浏览器 | 正文 + 全部评论 + 图片 | — |

**执行规则**：
- 只需正文和图片 → L1 即可
- 需要评论区 → **直接走 L2 MCP 浏览器**，`browser_navigate` 打开，`browser_evaluate` 提取 OP + 评论 + 图片 URL
...
**工具选择铁律：能跑脚本就跑脚本，MCP 浏览器是最后手段。**

| 优先级 | 工具 | 什么时候用 |
|--------|------|-----------|
| **L1** | `web_fetch.py` / `forum_scraper.py` | 始终最先尝试。覆盖绝大多数场景 |
| **L2** | `web_fetch_full.py`（本地 Playwright 脚本） | L1 失败且原因是需要 JS 渲染 |
| **L3** | MCP Playwright 浏览器 | **仅当 L1/L2 都无法解决时**。典型场景：知乎反爬（本地 Playwright 也 40362）、煎蛋评论区（JS 动态加载） |

> **原则**：L3 是例外，不是默认。每当你伸手去拿 MCP 浏览器之前，先问自己：脚本真的搞不定吗？如果脚本能搞定，就不要用浏览器。
>
> 反面教材：NGA 帖子有 `forum_scraper.py` + Cookie 完美支持，却用 MCP 浏览器去采——这就是违反铁律。今天批量采集的重复和内容错乱，根源就在此。
