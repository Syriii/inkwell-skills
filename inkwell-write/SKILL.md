...
4. **选文风**：从 `references/styles/` 中选择匹配的风格。根据体裁和人格匹配，而非简单的 category 匹配。若无匹配风格 → 提醒用户"没有匹配的风格，可以先用默认对话感写作，或触发文风采集"。用户选择采集时进入文风采集模式，走完整流程后再回到创作。
5. 用户确认 ✓
...
| 6 | **AI 味检测** | 对照选定风格文件的「禁忌清单」逐项检查（自问自答、编号列举、预告结构、总结句等） |
