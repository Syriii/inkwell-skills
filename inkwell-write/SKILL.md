...
提纲只是章节列表。写之前必须先推导出读者脑中的自然问题链——每个发现引出下一个好奇，每个段落是上一个问题的答案、也是下一个问题的起点。

**格式**：从开头到结尾，每个节点标注"读者这时候在想什么"，箭头指向下一段如何回应：
...
**检查**：
- 每个 ↓ 左边的段落**确实会**让读者产生右边的那个疑问
- 链条末尾必须落地到实际场景——读者看完知道怎么做，不是只知道怎么回事
- 如果某个 ↓ 左边的段落不会自然引发右边的问题，那就是提纲的逻辑顺序有问题
